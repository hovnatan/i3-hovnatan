// Derived from xcape.c https://github.com/alols/xcape
/************************************************************************
 * xcape.c
 *
...
int main(int argc, char **argv) {
  ...
  if (self->foreground != True) daemon(0, 0);
...
Key_t *key_add_key(Key_t *keys, KeyCode key) {
  ...
  if (keys == NULL) {
    keys = (Key_t *)malloc(sizeof(Key_t));
    rval = keys;
  } else {
    while (keys->next != NULL) keys = keys->next;
    keys = (keys->next = (Key_t *)malloc(sizeof(Key_t)));
  }
...
void intercept(XPointer user_data, XRecordInterceptData *data) {
  ...
  if (data->category == XRecordFromServer) {
    ...
    for (g = self->generated; g != NULL; g = g->next) {
      if (g->key == key_code) {
        if (self->debug) fprintf(stdout, "Ignoring generated event.\n");
        if (g_prev != NULL) {
          g_prev->next = g->next;
        } else {
...
KeyMap_t *parse_token(Display *dpy, char *token, Bool debug) {
  ...
  if (to != NULL) {
    ...
    for (;;) {
      ...
      if (!strncmp(key, "#", 1) && strsep(&key, "#") != NULL) {
        errno = 0;
      ...
      if (debug) {
        ...
        fprintf(stderr, "to \"%s\" (keysym 0x%x, key code %d)\n",
                XKeysymToString(ks_temp), (unsigned)ks_temp, (unsigned)code);
      }
    }
  } else
